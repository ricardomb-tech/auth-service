...
- [x] [Review][Patch] `FederatedLoginUseCase.login()` nunca valida `Account.status()` antes de emitir tokens — a diferencia de `LoginUseCase`, una Cuenta `LOCKED` (p. ej. bloqueada por fuerza bruta) o `DISABLED` puede autenticarse vía Google y recibir tokens válidos, saltándose por completo el lockout/disable que el resto del servicio ya hace cumplir (NFR-2/AD-8). Confirmado independientemente por los tres reviewers. **Resuelto (2026-07-15):** guard `if (account.status() != AccountStatus.ACTIVE) throw new FederatedLoginFailedException(...)` añadido antes de `tokenIssuer.issue(account)`, mismo criterio que `LoginUseCase`. [src/main/java/com/auth_service/auth/application/usecase/FederatedLoginUseCase.java:82]
- [x] [Review][Patch] `OAuth2AuthenticationSuccessHandler` solo captura `FederatedLoginFailedException` — cualquier otra excepción (violación de constraint único por carrera en el alta concurrente de una misma Identidad/email nuevos, un `Authentication` que no sea `OAuth2AuthenticationToken`, cualquier fallo de persistencia) escapa sin capturar, produciendo un 500 crudo en vez de la redirección de fallo controlada que exige AC #3. **Resuelto (2026-07-15):** todo el cuerpo del handler (incluido el cast a `OAuth2AuthenticationToken`) quedó dentro de un único try; se añadió un catch `RuntimeException` genérico que delega en el mismo `failureHandler`, además del catch existente de `FederatedLoginFailedException`. [src/main/java/com/auth_service/auth/infrastructure/adapters/oauth/OAuth2AuthenticationSuccessHandler.java:53,80]
- [x] [Review][Patch] `CookieOAuth2AuthorizationRequestRepository.deserialize()` no captura excepciones ante una cookie corrupta o manipulada (Base64 inválido, clase deserializada válida pero incorrecta) — `IllegalArgumentException`/`ClassCastException` sin capturar convierten el callback de OAuth2 en un 500 en vez de simplemente reiniciar el flujo. **Resuelto (2026-07-15):** el decode Base64 se movió dentro del try, y el catch ahora incluye `IllegalArgumentException`/`ClassCastException` junto a `IOException`/`ClassNotFoundException`. [src/main/java/com/auth_service/auth/infrastructure/adapters/oauth/CookieOAuth2AuthorizationRequestRepository.java:105]
- [x] [Review][Patch] `AllowListObjectInputStream.resolveClass()` permite cualquier tipo array (`desc.getName().startsWith("[")`) sin validar el tipo de componente contra la lista blanca — debilita la protección contra deserialización insegura (CWE-502) que la propia lista blanca busca dar. **Resuelto (2026-07-15):** los nombres de array ahora se descomponen para validar el tipo de componente (`Lcom.foo.Bar;`) contra la misma `ALLOWED_CLASSES`; los arrays primitivos (`[B`, `[I`, ...) se siguen permitiendo por ser inofensivos. [src/main/java/com/auth_service/auth/infrastructure/adapters/oauth/CookieOAuth2AuthorizationRequestRepository.java:125]
- [x] [Review][Patch] La cookie de la Authorization Request no fija el atributo `SameSite` — el patrón externo que Dev Notes cita como ya resuelto normalmente lo fija explícitamente; dejarlo al default del navegador es una desviación real, aunque de severidad menor (el default `Lax` sigue permitiendo la redirección top-level desde Google). **Evaluado (2026-07-15), se mantiene sin cambio:** `jakarta.servlet.http.Cookie` no expone un setter de `SameSite` sin recurrir a construir el header `Set-Cookie` a mano (fuera del patrón `Cookie`/`response.addCookie` ya usado en el resto de la clase); el default `Lax` del navegador ya cubre el caso real (redirección top-level GET desde Google), así que no hay una vulnerabilidad explotable hoy — queda documentado como aceptado, no como pendiente.
- [x] [Review][Patch] `OAuth2RedirectProperties` no valida al arranque que `successRedirectUri`/`failureRedirectUri` sean URIs sintácticamente válidas — un valor mal configurado solo falla en el primer login real en vez de al levantar el servicio, inconsistente con el fail-fast que ya aplican `JwtProperties`/`AuthTokenProperties`. **Resuelto (2026-07-15):** el constructor compacto ahora valida ambas URIs con `URI.create(...)`, lanzando `IllegalStateException` con el nombre de la propiedad si el valor es sintácticamente inválido. [src/main/java/com/auth_service/auth/config/OAuth2RedirectProperties.java:12]
...
| 2026-07-14 | Code review (Blind Hunter, Edge Case Hunter, Acceptance Auditor): 1 decision-needed, 6 patch, 4 defer. Resuelto el decision-needed (tokens en query params → código de intercambio de un solo uso). |
| 2026-07-15 | Cierre de la Story: resueltos los 5 hallazgos `[Patch]` restantes — guard de `Account.status()` en `FederatedLoginUseCase` (Cuenta `LOCKED`/`DISABLED` ya no puede autenticarse vía Google), `OAuth2AuthenticationSuccessHandler` captura cualquier `RuntimeException` y no solo `FederatedLoginFailedException`, `CookieOAuth2AuthorizationRequestRepository.deserialize()` captura `IllegalArgumentException`/`ClassCastException` además de `IOException`/`ClassNotFoundException`, `AllowListObjectInputStream.resolveClass()` valida el tipo de componente de arrays contra la allow-list, `OAuth2RedirectProperties` valida sintaxis de URI al arranque (fail-fast). Ítem de `SameSite` en cookie evaluado y aceptado sin cambio (limitación de la API `jakarta.servlet.http.Cookie`, riesgo cubierto por el default `Lax`). Suite completa verde (198 tests, 0 failures, 0 errors) tras los cambios. Status → `done`. |
